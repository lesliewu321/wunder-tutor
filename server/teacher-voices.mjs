...
export function createTeacherVoices({ azure, qwenKey = '', qwenRegion = 'singapore', chirpKey = '', cache, fetchImpl = fetch, maxGenerationsPerWindow = 120, windowMs = 600000 }) {
  ...
  const audible = wav => {
    let out;
    try { out = readWav(wav); } catch { throw new TtsError('voice_invalid_audio'); }
    if (!out.pcm.length || !out.pcm.some(byte => byte !== 0)) throw new TtsError('voice_no_audio');
    return wav;
  };
  async function generate(provider, req) {
    try {
      ...
      generated++;
      if (provider === 'azure') { const out = await azure(req); return audible(pcmToWav(out.pcm, out.rate)); }
      ...
      throw new TtsError(error?.name === 'TimeoutError' || error?.name === 'AbortError' ? 'voice_timeout' : 'voice_unavailable', 502);
    }
  }
  async function speak(input) {
    ...
    if (hit) {
      try { return result(audible(hit), true); } catch { memory.delete(key); }
    }
