...
// NOTE: written against Azure's documented response shape; not yet exercised against a live key.
...
export const mapAzure = (json: AzureResponse, referenceText: string, fallbackDurationMs: number): Assessment => {
  ...
  return {
    ...
    overall: Math.round(s.PronScore ?? s.AccuracyScore ?? 0),
...
const normalise = (ph: string): string => {
  ...
  const map: Record<string, string> = { 'ɹ': 'r', 'e': 'ɛ', 'ɡ': 'g', 'ɐ': 'ə', 'a': 'æ', 'ɒ': 'ɑ', 'əʊ': 'oʊ', 'ɜ': 'ɝ' };
  return map[p] ?? p;
};
