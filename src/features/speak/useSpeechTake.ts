...
import { blockCount } from '../../content/ko/hangul';
  ...
  const assess = async (rec: Recording) => {
    ...
    try {
      ...
      const assessment = await provider.assess(rec, j.item.text, {
        itemId: j.item.id, locale: localeOf(j.item, fresh.accent), accent: fresh.accent, zh: j.item.zh, ja: j.item.ja, ko: j.item.ko, script: fresh.zhScript, focus: j.item.focus,
