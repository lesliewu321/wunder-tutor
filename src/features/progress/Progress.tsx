...
const minutes = (ms: number) => (ms < 60000 ? `${Math.round(ms / 1000)}s` : `${Math.round(ms / 60000)} min`);

export function Progress() {
  const nav = useNavigate();
  const p = useActiveProfile();
  const t = totals(p.pronunciation);
    ...
    <div className="screen progress">
      ...
      <section className="card trend">
        <div className="trend__head">
          <div><small>Pronunciation</small><b className={`score-text score-text--${tier(today.avg)}`}>{today.avg}</b></div>
          <p>{summary
            ? summary.to > summary.from ? <>Up from <b>{summary.from}</b> to <b>{summary.to}</b> this month. Keep going!</> : <>Holding steady around <b>{summary.to}</b>. A little practice each day moves it.</>
            : <>Today’s average. Come back tomorrow to start your trend line.</>}</p>
        </div>
        <TrendChart points={points} />
      </section>

      <div className="stat-grid">
        <div className="stat"><b>{lessons}</b><span>of {ALL_LESSONS.length} lessons</span></div>
        <div className="stat stat--coral"><b>{minutes(t.speakingMs)}</b><span>speaking out loud</span></div>
        <div className="stat stat--leaf"><b>{t.wordsLearned}</b><span>words said clearly</span></div>
        <div className="stat stat--sun"><b>{Math.max(liveStreak(p.streak), 0)}</b><span>day streak · best {p.streak.best}</span></div>
      </div>

      <section>
        <h2 className="section-title">Sounds that need work</h2>
        {weak.length ? (
      ...
      <section>
        ...
        <div className="trophies">
          {ACHIEVEMENT_CATALOGUE.map((a) => {
            ...
            return <div key={a.id} className={`trophy ${got ? 'is-on' : ''}`} title={a.detail}><span>{got ? a.icon : '🔒'}</span><small>{a.title}</small></div>;
          })}
        </div>
      </section>
    </div>
...
function TrendChart({ points }: { points: TrendPoint[] }) {
  ...
  if (points.length < 2) {
    return <div className="trend__single"><div className="trend__dot" style={{ bottom: `${points[0].avg * 0.7}%` }} /><span>Day 1</span></div>;
  }
  ...
  return (
    <svg className="trend__chart" viewBox={`0 0 ${W} ${H}`} role="img" aria-label={`Pronunciation from ${points[0].avg} to ${points[points.length - 1].avg}`}>
